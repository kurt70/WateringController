...
**Shared state rule** — `.squad/` is versioned repository state unless a file is explicitly machine-local and ignored. Decisions, routing, logs, and roster updates should be committed so the same team state is available across machines.
...
- Agents should trust the provided `TEAM_ROOT` when present and only fall back to local discovery if it is missing.
...
| `.squad/decisions.md` | **Authoritative decision ledger.** Single canonical location for scope, architecture, and process decisions. | Scribe (merge from inbox), Squad (Coordinator) may edit governance-only corrections | All agents |
...
4. Non-coordinator agents may propose decisions in their responses, but accepted decisions are written to the inbox and merged into `.squad/decisions.md` by Scribe.
